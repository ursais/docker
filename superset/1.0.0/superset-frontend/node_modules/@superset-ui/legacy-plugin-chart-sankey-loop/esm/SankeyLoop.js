...
import './SankeyLoop.css'; // a problem with 'd3-sankey-diagram'  is that the sankey().extent() paramters, which
...
function SankeyLoop(element, props) {
  ...
  const layout = sankey().nodeId(d => d.id).extent([[margin.left, margin.top], [innerWidth, innerHeight]]);
  const diagram = sankeyDiagram().nodeTitle(d => d.name).linkTitle(({
    source: {
      name: sName,
      value: sValue
    },
  ...
  const svg = select(element).append('svg').classed('superset-legacy-chart-sankey-loop', true).style('width', width).style('height', height).datum(layout(computeGraph(data))).call(diagram);
